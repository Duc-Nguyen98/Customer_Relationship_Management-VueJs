<template>
    ...
    <div style="height: inherit">
            ...
            <div class="app-action">
                ...
                >
                    ...
                    <feather-icon
                            ...
                            size="17"
                            class="cursor-pointer ml-1"
                            @click="moveSelectedEmailsToFolder('delete')"
                    />
                    ...
                    >
                        ...
                        <b-media-aside class="media-left mr-50">
                            <b-avatar
                                    ...
                                    size="40"
                                    variant="primary"
                                    :src="email.from && email.from.avatar != null ? email.from.avatar : ''"
                            />
                        ...
                        <b-media-body>
                            <div class="mail-details">
                                <div class="mail-items">
                                    <h5 class="mb-25">
                                        {{ email.from && email.from.name != null ? email.from.name : '' }}
                                    </h5>
        ...
        <!-- Compose Email Modal -->
        <email-compose v-model="shallShowEmailComposeModal" @send-email="sendEmail" />
    </div>
</template>
<style scoped>
    .b-pagination {
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            // eslint-disable-next-line no-use-before-define
            watch([searchQuery, page, perPage], () => fetchEmails())
            ...
            const selectAllCheckboxUpdate = val => {
                selectedEmails.value = val ? emails.value.map(mail => mail._id) : []
            }
            ...
            const moveSelectedEmailsToFolder = folder => {
                if (folder != 'delete') {
                    store.dispatch('app-email/updateEmail', {
                        ...
                        .then((response) => {
                            if (response.data.success) {
                                alert("success", `Change ${folder} successfully.`)
                                fetchEmails()
                            } else {
                                alert("danger", `Change ${folder} failed.`)
                            }
                        })
                        .finally(() => {
                    ...
                    store.dispatch('app-email/deleteEmail', {
                        emailIds: selectedEmails.value,
                    })
                        .then((response) => {
                            if (response.data.success) {
                                alert("success", `Delete email successfully.`)
                                fetchEmails()
                            } else {
                                alert("danger", `Delete email failed.`)
                            }
                        })
                        .finally(() => {
                            selectedEmails.value = []
            ...
            const moveOpenEmailToFolder = folder => {
                selectedEmails.value = [emailViewData.value._id]
            ...
            const changeOpenedEmail = dir => {
                const openedEmailIndex = emails.value.findIndex(e => e._id === emailViewData.value._id)
            ...
            const sendEmail = value => {
                if (value) {
                    alert("success", "Create email successfully.")
                    fetchEmails()
                } else {
                    alert("danger", "Create email failed.")
                }
            }
